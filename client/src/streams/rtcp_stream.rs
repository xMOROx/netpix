...
impl RtcpStream {
            ...
            RtcpPacket::SenderReport(report) => {
                if report.ssrc == self.ssrc {
                        ...
                        (self.last_sr_timestamp, self.last_sr_octet_count)
                    {
                        ...
                        if let Some(delta_duration) = current_event_time_duration.checked_sub(&last_event_time_duration) {
                            let delta_time_secs = delta_duration.num_microseconds().unwrap_or(0) as f64 / 1_000_000.0;
    ...
    pub fn update_with_rr(&mut self, timestamp: time::Duration, report: &ReceptionReport) {
       ...
       self.jitter_history.push(PlotPoint::new(timestamp.as_secs_f64(),report.jitter as f64));
